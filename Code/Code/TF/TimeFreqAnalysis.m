...
TFparams.mapsparams = SetMapsParams(Nav,Spk);
...
occmap = ComputeMap(spd_discrete(tidx), [], flat(tidx), nspdbins, []);

%Computing the sum of theta power and theta frequency across speeds
ThetaPowSmap = ComputeMap(spd_discrete(tidx), [], thetapow(tidx), nspdbins, []);
ThetaFqSpdmap = ComputeMap(spd_discrete(tidx), [], thetafreq(tidx), nspdbins, []);
